...
class MapViewPageScene: SKScene {
    ...
    func showHomeButton() {
        ...
        homeBtn.position = CGPoint(x: self.frame.minX + 100, y: self.frame.maxY - 100)
        addChild(homeBtn)
    }
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for touch in touches {
            ...
            if let name = node.name, name.contains("_challenge") {
                ...
                let selectedChallenge = self.theme?.challenges?.filtered(
                    using: NSPredicate(format: "challengeName == %@", name)
                ).array[0] as? Challenges

                if !(selectedChallenge?.isActive ?? false) {
                    return
                }
                    ...
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.15) {
                        ...
                        if let sceneNode = scene.rootNode as? StartPageScene? {
                            ...
                            if let view = self.view {
                                ...
                                view.presentScene(sceneNode ?? SKScene(),
